function initLineChart() {
    ...
    xRange = d3.time.scale().range([MARGINS.left, WIDTH1 - MARGINS.right])
        .domain([new Date(jsonArray[0].timestamp_s), d3.time.minute.ceil(new Date(jsonArray[jsonArray.length-1].timestamp_s ))
        ]),
        ...
        .domain([Math.floor(d3.min(jsonArray, function (d) {
            ...
            }) / 200) * 200, Math.ceil(d3.max(jsonArray, function (d) {
    ...
    svgLine.append("text")
        .attr("transform", "translate(" + (WIDTH1 - 40) + " ," + (scalefactor + 75) + ")")
    ...
    var lineFunc0 = d3.svg.line()
        .x(function (d) {
            return xRange(new Date(d.timestamp_s));
        })
        .y(function (d) {
